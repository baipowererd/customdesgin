...
import { Code2, Monitor, Copy, Check, Palette, Type, X, Info, Atom, FileJson } from 'lucide-react';
...
export function useColorState(defaults) {
    const [primary, setPrimary] = useState(defaults?.primary || '#6366f1');
    const [secondary, setSecondary] = useState(defaults?.secondary || '#818cf8');
    return {
        primary, setPrimary,
        secondary, setSecondary,
        colors: { primary, secondary, dark: darken(primary), light: lighten(primary) },
    };
}

const ThemeShowcase = ({ title, description, cssTemplate, renderPreview, multiColor, matchedTags, onTagClick }) => {
    const [activeTab, setActiveTab] = useState('preview');
    const [copied, setCopied] = useState(false);
    const { primary, setPrimary, secondary, setSecondary, colors } = useColorState();
    ...
    // format CSS with line breaks for readability
    const formattedCSS = generatedCSS.replace(/;/g, ';\n');

    // Generate dynamic React code
    const generatedReactCode = `/* React Component */
const ${title.replace(/\s+/g, '')} = () => {
  return (
    <div className="preview-canvas">
      ...
      <div 
        style={{ 
          fontSize: "${editSettings.fontSize || 'default'}",
          fontWeight: "${editSettings.fontWeight || 'default'}",
          fontFamily: "${editSettings.fontFamily || 'inherit'}"
        }}
      >
        ${editSettings.text || '/* Component Content goes here */'}
      </div>
    </div>
  );
};`;
        ...
        <div className="showcase-card">
            <div className="showcase-header">
                ...
                <div className="showcase-tabs">
                    ...
                    <button className={`tab-btn ${isEditing ? 'active' : ''}`} onClick={() => { setActiveTab('preview'); setIsEditing(!isEditing); }} title="Live Edit Text">
                        <Type size={16} /> Edit
                    </button>
                </div>
            </div>
                            ...
                            <div className="edit-modal-backdrop" onClick={() => setIsEditing(false)}>
                                <div className="edit-modal-content" onClick={e => e.stopPropagation()}>
                                    ...
                                    <div className="edit-modal-left">
                                        <div className={`preview-canvas ${canvasId}`}>
                                            ...
                                            {renderPreview({ ...colors, text: editSettings.text })}
                                        </div>
                                    </div>
                        ...
                        <div className="color-bar">
                            ...
                            <div className="color-swatches">
                                {presetPalettes.map((p) => (
                                    <button
                                        key={p.name}
                                        className={`swatch ${primary === p.primary ? 'active' : ''}`}
                                        style={{ background: p.primary }}
                                        title={p.name}
                                        onClick={() => applyPreset(p)}
                                    />
                                ))}
                                <button className="swatch swatch-custom" title="Custom color" onClick={() => setShowPicker(!showPicker)}></button>
                            </div>
                        </div>
